...
import { capitalize, joinQuestion } from "app/functions/Utils";
...
export default function Description() {
  ...
  const fetchQuestion = async () => {
    try {
      console.log(joinQuestion(problem));
  ...
  return (
    <div className="flex flex-col gap-2 p-4">
      ...
      <h1
        className={`font-bold
          ${question.difficulty.toLowerCase() === "medium" && "text-yellow-400"}
          ${question.difficulty.toLowerCase() === "easy" && "text-green-400"}
          ${question.difficulty.toLowerCase() === "hard" && "text-red-500"}`}
      >
      ...
      {/* Topics */}
      {question.topics && (
        <div className="w-full overflow-hidden border-b-2 border-[#353535] cursor-pointer mt-2">
          <div
          ...
          >
            <div className="flex flex-wrap gap-2 px-4 py-3">
              {question.topics.map((item, index) => (
                <div
                  key={index}
                  className="text-[13px] text-gray-300 font-semibold px-4 py-1 rounded-full bg-[#353535]"
                >
                  {capitalize(item)}
                </div>
              ))}
      ...
      {question.hints && question.hints.length > 0 && (
        <div className="flex flex-col gap-2">
          {question.hints.map((item, index) => (
            <div
              key={index}
              className="w-full overflow-hidden border-b-2 border-b-[#353535]"
            >
              <div
                onClick={() => toggleCollapse(index)}
                className="flex py-2 cursor-pointer justify-between px-4"
              >
                <div className="flex gap-4 items-center text-[#c9c9c9]">
                  <Lightbulb />
